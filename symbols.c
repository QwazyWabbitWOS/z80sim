...
source SearchSource(word Address) {
	//source SourceLine;
	word i = Address;
	while (SourceCode[i].File == NULL && i > 0)
		i--;
	return
		SourceCode[i];
}
...
logic HasSource(word Address) {
	if (SourceCode[Address].File != NULL)
		return TRUE;
	else
		return FALSE;
}
...
word GetSymbol(const char* Name) {
	...
	int i;
	if (Symbols[CachedAddress])
		if (!strcmp(Symbols[CachedAddress], Name))
			return CachedAddress;
	for (i = 0; i < SIZE_MEMORY; i++) {
		if (Symbols[i])
			if (!strcmp(Symbols[i], Name))
				return CachedAddress = (word)i;
	}
	return 0;
}
